...
После долгих поисков, я остановился на [Ksnip](https://github.com/ksnip/ksnip) (более-менее сносно). Это приложение использует библиотеки Qt, поэтому будет не лишним воспользоваться [Qt адаптером](/eos-all-about/blob/main/after-install.md#подключить-поддержку-тем-gtk-для-qt-приложений).

⚠️ Рекомендую [скачать deb пакет](https://github.com/ksnip/ksnip/releases) и установить из него.

Через репозиторий можно устанавливать только для систем на UBUNTU до 21 версии.
...
Я не установил только по той причине, что у меня прога работала со странностями (при выделении экрана не видно рамку, очень неудобно).
